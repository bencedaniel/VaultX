...
describe('editHorse.ejs', () => {
  const templatePath = path.join(__dirname, '../../views/horse/editHorse.ejs');
  let template;
  ...
  it('renders dropdown options for sex', async () => {
    const html = await render();
    expect(html).toContain('Mare');
    expect(html).toContain('Gelding');
    expect(html).toContain('Stallion');
  });

  it('renders dropdown options for status', async () => {
    const html = await render();
