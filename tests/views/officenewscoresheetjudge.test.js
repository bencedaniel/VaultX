...
const templatePath = path.resolve(process.cwd(), 'views/scoringJudge/officenewscoresheetjudge.ejs');

function renderOfficeNewScoresheetJudge(overrides = {}) {
  const data = {
    scoresheetTemp: {
      _id: 'tmpl-1',
...
describe('views/scoringJudge/officenewscoresheetjudge.ejs', () => {
  test('renders form action, template image and submit button', async () => {
    const html = await renderOfficeNewScoresheetJudge();

    expect(html).toContain('action="/scoring/office/newscoresheet"');
  ...
  test('renders hidden metadata fields and uses judgeID for judge user id', async () => {
    const html = await renderOfficeNewScoresheetJudge({ judgeID: 'judge-99' });
    ...
    expect(html).toContain('name="Judge[userId]" value="judge-99"');
