...
const templatePath = path.resolve(process.cwd(), 'views/scoringJudge/newscoresheetjudge.ejs');

function renderNewScoresheetJudge(overrides = {}) {
  const data = {
    scoresheetTemp: {
      _id: 'tmpl-1',
...
describe('views/scoringJudge/newscoresheetjudge.ejs', () => {
  test('renders form action, template image and submit button', async () => {
    const html = await renderNewScoresheetJudge();

    expect(html).toContain('action="/scoring/newscoresheet"');
  ...
  test('renders hidden metadata fields and uses user._id for judge user id', async () => {
    const html = await renderNewScoresheetJudge({ user: { _id: 'user-77' } });
    ...
    expect(html).toContain('name="Judge[userId]" value="user-77"');
  ...
  test('includes common runtime script wiring and static scoresheet script include', async () => {
    ...
    expect(html).toContain("console.log('Date for output:', date);");
  ...
  test('contains enter-key blocking handler for input-field elements', async () => {
    const html = await renderNewScoresheetJudge();

    expect(html).toContain("document.querySelectorAll('.input-field').forEach(input => {");
  ...
  test('renders horse config values when typeOfScores is horse', async () => {
    const html = await renderNewScoresheetJudge({
      scoresheetTemp: {
        _id: 'tmpl-horse',
    ...
    // In this template A1 intentionally has no || 0 fallback
